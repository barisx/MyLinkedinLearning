...
class PackageCreateView(CreateAPIView):
    ...
    def post(self, *args, **kwargs):
        if cache.has_key('package_created')  # package create key?
            return Response(status=200)  # return success response
        response = super().post(*args, **kwargs)  # get latest response
        if response.status_code == 201:  # if create response code
            cache.set('package_created', True, timeout=300)   # cache it as package created for 0.3 sec
        return response

class Handler:  # Abstract Handler
    ...
    def handle(self, request): # class Handler
...
class ConcreteHandler1(Handler):  # Inherits from the abstract handler
    """
        Concrete handler 1
    """
    def _handle(self, request): # class ConcreteHandler1
...
class DefaultHandler(Handler):  # Inherits from the abstract handler
    ...
    def _handle(self, request):  # Default Handler
        """
            If there is no handler available
        """
        # No condition checkingf since this is a default handler
